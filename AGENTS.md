...
- SDK 按 services、messaging、packaging 分组，合同与客户端按功能内聚；不新增根 services.py
  大杂烩或单方法文件。卡片 UI、字段映射、渠道限制及资产校验均由插件拥有。
- 新平台服务必须同时实现平台授权与 SDK 调用，并迁移全部调用方；不能只声明客户端。
  安装获批权限与实时用户权限取交集，禁止提供平台数据库连接或绕过工具网关直接执行。
