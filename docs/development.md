...
状态：2026-09-20；SDK 0.5.0、钉钉插件 0.3.0 已通过 GitHub Release 发布，
不宣称已在 PyPI 发布；插件双架构镜像和签名描述文件已发布，真实渠道验收未完成。
后续变更须先走 PR。发布签名 Secret 已配置，
...
钉钉 Dockerfile 固定基础镜像摘要并以非 root 用户运行。镜像已发布，不能把
构建成功或模拟 Docker 流程当作真实渠道验收。目录发布见 [catalog](catalog.md)。
